...
import type { ConventionRecord, NewConvention } from './types.js';
 ...
 * decides which of its candidates survive, clustering, and the skill markdown
 * assembled from the accepted ones. No IO, no model, no DB.
 */
 ...
 * mechanically checkable afterwards.
 */
...
export interface VerifiedCandidate {
  ...
  /** Sampled files whose text contains the snippet head (code-verified support). */
  supportFiles: string[];
  confidence: number;
}
 ...
 *  1. the cited path is one we sampled (a path we never sent is invented, and
 *     a `../` traversal can never match);
...
export function verifyCandidate(files: Map<string, SampledFile>, c: RawCandidate): VerifyResult {
  ...
  return {
    ...
    candidate: {
      ...
      supportFiles: supportingFiles(files, head, file.path),
 ...
 * meant would defeat the gate), and any `..` segment is refused outright.
 */
...
/**
 * Code-verified support: every sampled file that contains the snippet head,
 * the cited file first. This replaces the model's self-reported count, which
 * grounds the "seen in N files" chip in fact instead of a flat 0.90.
 */
function supportingFiles(
  ...
  const needle = norm(head);
  const out = [primaryPath];
  for (const f of files.values()) {
    if (f.path === primaryPath) continue;
    if (f.lines.some((l) => norm(l).includes(needle))) out.push(f.path);
  }
  return out;
}
...
/**
 * Merge candidates that state the same rule (same `ruleKey`) into ONE card.
 *
 * A model that sees the same pattern in four files tends to return four
 * identical cards; one decision should cover them all. The highest-confidence
 * member is the primary (its snippet is what the card shows); the others'
 * files join the evidence list and lift `occurrences`. Output is sorted by
 * confidence, strongest first.
 */
export function clusterCandidates(candidates: VerifiedCandidate[]): {
  clustered: NewConvention[];
  merged: number;
} {
  ...
  const clustered: NewConvention[] = [];
  for (const group of groups.values()) {
    const primary = group[0]!;
    const files: string[] = [];
    for (const g of group) {
      for (const p of [g.evidencePath, ...g.supportFiles]) if (!files.includes(p)) files.push(p);
    }
    clustered.push({
      category: primary.category,
      rule: primary.rule,
      rationale: primary.rationale,
      evidencePath: primary.evidencePath,
      evidenceLine: primary.evidenceLine,
      evidenceSnippet: primary.evidenceSnippet,
      evidenceFiles: files.slice(0, MAX_EVIDENCE_FILES),
      occurrences: files.length,
      confidence: primary.confidence,
    });
  }
  return { clustered, merged: candidates.length - clustered.length };
}

/** Drop clustered candidates whose rule the user already decided (accepted or rejected). */
export function dropDecided(
  candidates: NewConvention[],
  decidedKeys: Iterable<string>,
): { kept: NewConvention[]; dropped: number } {
  const keys = new Set(decidedKeys);
  const kept = candidates.filter((c) => !keys.has(ruleKey(c.rule)));
...
/** Candidates the list endpoint returns: rejected rows stay in the DB but are hidden. */
export function isVisible(row: ConventionRecord): boolean {
 ...
 * user edits before saving: a starting point, not a final artifact.
 *
...
export function buildSkillDraft(
  ...
  const evidenceFiles = [
    ...new Set(accepted.flatMap((r) => (r.evidencePath ? [r.evidencePath] : []))),
  ];

  return {
    name: CONVENTIONS_SKILL_NAME,
    description: `${accepted.length} house convention${accepted.length === 1 ? '' : 's'} extracted from ${repoFullName}`,
    type: 'convention',
    body,
