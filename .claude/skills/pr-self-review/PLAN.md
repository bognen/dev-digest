...
> **Re-adopted for homework 2 (P8, criteria 5 and 21).** The skill is restored
> as a **Workflow (dispatcher)** skill. Changes vs the 2026-08 version: routing
> targets `frontend-architecture` (renamed from `frontend-ui-architecture`);
> `client/**` and `server/**` skill sets are stated explicitly so a mixed diff
> loads both; `pnpm arch` became `pnpm arch:check`; the vendor-sync and
> prompt-sync scripts are now Phase 2 gates; `CLAUDE.md` references became
> `AGENTS.md`; frontmatter gained `disable-model-invocation: true`; and the
> skill is disambiguated from the seeded *app* agent also named `pr-self-review`.
> **No auto-invoke hook is installed** (criterion 21) — see the opt-in snippet at
...
|---|---|
| `client/src/**/*.{ts,tsx}` | `frontend-architecture` (was `frontend-ui-architecture`), `react-best-practices` |
...
2. **Hand-edited migration** — diff modifies an *existing* file under
   ...
   are not (`server/AGENTS.md` do-not-touch).
...
## Opt-in: auto-invoke on git push (not installed)

**Status: deliberately NOT installed.** `.claude/settings.json` contains no
`git push` / `gh pr create` hook, so `/pr-self-review` only ever runs when you
type it (criterion 21: auto-invoke is off; the manual run on a mixed diff is the
supported path). A hook here would affect every Claude Code session in the repo,
not just self-review runs, and needs an escape hatch first.

If you *want* the hard local gate (layer B above), add the following yourself.
It is a documented opt-in, untested against this repo — try it on a scratch
branch first.
    ...
    "PreToolUse": [
      {
        "matcher": "Bash",
        "hooks": [
          { "type": "command", "command": "bash .claude/hooks/pr-self-review-gate.sh" }
        ]
      }
    ]
  }
...
2. `.claude/hooks/pr-self-review-gate.sh` (exit code 2 blocks the tool call and
   feeds stderr back to the agent):

```bash
#!/usr/bin/env bash
# Blocks `git push` / `gh pr create` unless a fresh, non-BLOCKED verdict exists.
# Escape hatch: PR_SELF_REVIEW_SKIP=1 in the environment.
cmd=$(node -e 'let d="";process.stdin.on("data",c=>d+=c).on("end",()=>{try{console.log(JSON.parse(d).tool_input.command||"")}catch{}})')
case "$cmd" in "git push"*|"gh pr create"*) ;; *) exit 0 ;; esac
[ "${PR_SELF_REVIEW_SKIP:-}" = "1" ] && exit 0

V="$(git rev-parse --git-dir)/pr-self-review-verdict.json"
[ -f "$V" ] || { echo "No pr-self-review verdict yet - run /pr-self-review first." >&2; exit 2; }

BASE=$(git merge-base main HEAD)
HEAD_SHA=$(git rev-parse HEAD)
TREE=$( { git diff "$BASE"; git ls-files --others --exclude-standard | sort | xargs -r git hash-object; } | git hash-object --stdin )
node -e '
  const v=require(process.argv[1]);
  const ok=v.verdict!=="BLOCKED" && v.head===process.argv[2] && v.treeHash===process.argv[3];
  if(!ok){console.error("pr-self-review verdict is BLOCKED or expired - re-run /pr-self-review.");process.exit(2)}
' "$V" "$HEAD_SHA" "$TREE"
```

Remember layer C (branch protection + a required CI check) is still the only
thing that can forbid a *merge*; this hook only guards the local push.
