...
Each one is a mistake this repo actively invites, documented in `AGENTS.md`,
...
| 2 | hand-edited migration | `AGENTS.md` do-not-touch |
| 3 | schema without migration | drizzle-kit generates; a schema edit alone never reaches the DB |
| 4 | module not registered | `AGENTS.md` — static registration in `server/src/modules/index.ts`, no autoload |
| 5 | ESM `.js` extension | `AGENTS.md` — required in `server/`+`reviewer-core/`, forbidden in `client/` |
